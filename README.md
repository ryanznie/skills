...
Each plugin should keep its own semver in its `plugin.json`. GitHub releases are created by [release.yml](.github/workflows/release.yml), which lets you choose the target plugin, bumps that plugin version, commits it on a release branch, creates the matching `<plugin>@<version>` tag, publishes the release from that exact version, and opens a PR back to `main`.
