...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Optional .env-style file to source (only fills missing env vars). Default: plugins/calendar-skills/skills/ai-scheduler/.env.scheduler",
    )
...
def main() -> None:
    ...
    if args.no_subject_prefix:
        ...
        if not subject_prefix:
            raise SystemExit(
                "Missing subject prefix. Set AGENTMAIL_SUBJECT_PREFIX in plugins/calendar-skills/skills/ai-scheduler/.env.scheduler (or pass --subject-prefix), "
