 ...
 *
 * Usage:
 *   bun validate-frontmatter.ts
 *   bun validate-frontmatter.ts /path/to/dir
 *   bun validate-frontmatter.ts file1.md file2.md
 */
...
async function main() {
  ...
  if (args.length > 0 && args.every((arg) => arg.endsWith(".md"))) {
    baseDir = process.cwd();
    files = [];
  ...
  if (totalErrors === 0 && totalWarnings === 0) {
    console.log("All frontmatter files look valid.");
  } else {
    console.log(`Done with ${totalErrors} error(s) and ${totalWarnings} warning(s).`);
  }
